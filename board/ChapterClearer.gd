...
func unlock_next_chapter(current_chapter_title: String, next_chapter_title: String) -> void:
	var save_data := GameData.save_data
	
	var current_chapter_save_data = save_data.find_unlocked_chapter_by_title(current_chapter_title)
	
	assert(current_chapter_save_data != null)
	
	if current_chapter_save_data.is_cleared:
		# If already cleared, don't do anything
		print("Already finished %s" % current_chapter_title)
	else:
		# Mark as cleared
		current_chapter_save_data.is_cleared = true
		
		# Add next level to list of unlocked levels
		save_data.unlock_chapter(next_chapter_title)
